...
var Audio = (function(){
    ...
    me.playSample = function(index,period,volume,track,effects,time,noteIndex){
		...
		if (instrument){
            ...
            if (Tracker.inFTMode() && instrument.sample.c2spd && !Tracker.useLinearFrequency){
                sampleRate = sampleRate * (instrument.sample.c2spd / 8363);
            }
	...
	me.playSynth = function(index,period,volume,track,effects,time,noteIndex){
		...
		period = period || 428; // C-4 in FastTracker/S3M mode
		if (typeof track === "undefined") track = (hasUI ? Editor.getCurrentTrack() : 0);
		time = time || context.currentTime;
